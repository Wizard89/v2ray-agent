...
acmeInstallSSL() {
    if [[ "${dnsSSLStatus}" == "true" ]]; then
        ...
        if [[ -n "${txtValue}" ]]; then
			...
			echoContent yellow " ---> 添加方法请参考此教程，https://github.com/Wizard89/v2ray-agent/blob/master/documents/dns_txt.md"
...
EOF
        ...
        cat <<EOF >>"/etc/v2ray-agent/subscribe_local/default/${user}"
tuic://${id}:${id}@${currentHost}:${tuicPort}?congestion_control=${tuicAlgorithm}&alpn=h3&sni=${currentHost}&udp_relay_mode=quic&allow_insecure=0#${email}
EOF
	...
	if echo "${currentInstallProtocolType}" | grep -q 0 && [[ -n "${configPath}" ]]; then
        ...
        if [[ -n $(ls /etc/v2ray-agent/subscribe_local/default/) ]]; then
            find /etc/v2ray-agent/subscribe_local/default/* | while read -r email; do
				...
				echoContent yellow "url:https://${currentDomain}/s/${emailMd5}\n"
