...
hysteriaPortHopping() {
    ...
    if [[ -z "${hysteriaPortHoppingRange}" ]]; then
        ...
        if [[ -z "${portStart}" || -z "${portEnd}" ]]; then
            ...
            iptables -t nat -A PREROUTING -p udp --dport "${portStart}:${portEnd}" -m comment --comment "Wizard89_hysteria2_portHopping" -j DNAT --to-destination :${hysteriaPort}

            if iptables-save | grep -q "Wizard89_hysteria2_portHopping"; then
...
readHysteriaPortHopping() {
    if [[ -n "${hysteriaPort}" ]]; then
        if iptables-save | grep -q "Wizard89_hysteria2_portHopping"; then
            portHopping=
            portHopping=$(iptables-save | grep "Wizard89_hysteria2_portHopping" | cut -d " " -f 8)
...
deleteHysteriaPortHoppingRules() {
    iptables -t nat -L PREROUTING --line-numbers | grep "Wizard89_hysteria2_portHopping" | awk '{print $1}' | while read -r line; do
